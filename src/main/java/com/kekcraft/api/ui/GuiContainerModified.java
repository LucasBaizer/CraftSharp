...
abstract class GuiContainerModified extends GuiScreen {
	...
	public GuiContainerModified(Container p_i1072_1_) {
	...
	public void drawScreen(int p_73863_1_, int p_73863_2_, float p_73863_3_) {
		...
		outer: for (int i1 = 0; i1 < this.inventorySlots.inventorySlots.size(); ++i1) {
			Slot slot = (Slot) this.inventorySlots.inventorySlots.get(i1);

			for (Class<? extends Slot> clazz : disallow) {
				if (clazz.isAssignableFrom(slot.getClass())) {
					continue outer;
				}
			}

			boolean b = false;
			for (Class<? extends Slot> clazz : allow) {
				if (clazz.isAssignableFrom(slot.getClass())) {
					b = true;
					break;
				}
	...
	private void func_146977_a(Slot p_146977_1_) {
		...
		if (itemstack == null) {
			...
			if (iicon != null) {
				GL11.glDisable(GL11.GL_LIGHTING);
				GL11.glEnable(GL11.GL_BLEND); // Forge: Blending needs to be
												// enabled for this.
	...
	private void func_146980_g() {
		...
		if (itemstack != null && this.field_147007_t) {
			...
			for (Iterator<Slot> iterator = this.field_147008_s.iterator(); iterator
	 ...
	 */
	private Slot getSlotAtPosition0(int p_146975_1_, int p_146975_2_) {
	...
	protected ArrayList<Class<? extends Slot>> allow = new ArrayList<Class<? extends Slot>>();
	protected ArrayList<Class<? extends Slot>> disallow = new ArrayList<Class<? extends Slot>>();

	public Slot getSlotAtPosition(int x, int y) {
		Slot slot = this.getSlotAtPosition0(x, y);
		if (slot == null) {
			return null;
		}
		for (Class<? extends Slot> clazz : disallow) {
	...
	protected void mouseMovedOrUp(int p_146286_1_, int p_146286_2_, int p_146286_3_) {
		super.mouseMovedOrUp(p_146286_1_, p_146286_2_, p_146286_3_); // Forge,
																		// Call
																		// parent
																		// to
																		// release
