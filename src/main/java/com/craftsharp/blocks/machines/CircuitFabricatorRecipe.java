...
import com.craftsharp.api.ui.IMachineSmeltableRecipe;
...
public class CircuitFabricatorRecipe implements IMachineSmeltableRecipe {
	...
	public boolean satifies(MachineTileEntity entity, ItemStack[] slots) {
		return has(slots[0], CraftSharp.factory.getItem("RefinedSilicon")) && slots[1] != null
	...
	public boolean isValidElementOfRecipe(ItemStack element, int index) {
		return ((index == 0 || index == 2) && element.getItem() == CraftSharp.factory.getItem("RefinedSilicon"))
	...
	public ItemStack getInput(int slot) {
		return slot == 0 ? new ItemStack(CraftSharp.factory.getItem("RefinedSilicon")) : (slot == 1 ? inputA : inputB);
	}
